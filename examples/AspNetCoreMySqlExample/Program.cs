...
app.MapGet("/", () => "DurableStack AspNetCore MySQL Example");
...
public sealed class SendWelcomeEmailJob : IDurableJob<SendWelcomeEmailArgs>
{
    ...
    public Task ExecuteAsync(SendWelcomeEmailArgs args, JobContext context, CancellationToken cancellationToken)
    {
        _logger.LogInformation(
            "[MySqlExample] Sending welcome email to {Email}. RunId={RunId} Attempt={Attempt}",
            args.Email,
            context.RunId,
            context.Attempt);
...
public sealed class HeartbeatJob : IDurableJob
{
    ...
    {
        _logger.LogInformation("[MySqlExample] Recurring heartbeat executed. RunId={RunId}", context.RunId);
        return Task.CompletedTask;
    }
}
...
public sealed class LongRunningLeaseDemoJob : IDurableJob
{
    ...
    {
        _logger.LogInformation("[MySqlExample] Long-running lease demo started. RunId={RunId}", context.RunId);
        await Task.Delay(TimeSpan.FromSeconds(20), cancellationToken);
        _logger.LogInformation("[MySqlExample] Long-running lease demo completed. RunId={RunId}", context.RunId);
    }
}
