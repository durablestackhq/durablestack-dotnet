...
internal static class SqlServerTableNameResolver
{
    ...
    private static string Build(string? prefix, string baseName)
    {
        ...
        if (normalizedPrefix.Length > MaxPrefixLength)
        {
            throw new ArgumentException(
                $"DatabaseTablePrefix may be at most {MaxPrefixLength} characters for SQL Server providers.",
                nameof(prefix));
        }
        ...
        {
            throw new ArgumentException(
                "DatabaseTablePrefix may only contain letters, digits, and underscores for SQL Server providers (semicolons and other punctuation are not allowed).",
                nameof(prefix));
        }
